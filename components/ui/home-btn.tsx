...
function HomeBtn({ className,children}:{className:string,children:React.ReactNode}) {
  ...
  <span className={` ${className} font-montserrate inline-flex h-full w-full cursor-pointer items-center justify-center rounded-full  px-5 py-1 text-sm font-medium text-white backdrop-blur-3xl`}>
   {children}
  </span>
</button>
  )
